...
execute as @e[type=minecraft:armor_stand,tag=pot_marker,tag=!pot_marker_item_set] at @s if data block ~ ~ ~ {item:{id:"minecraft:decorated_pot"}} run function enhanced_pots:summon_pot_overflow
...
# item added to pot
execute as @e[type=minecraft:armor_stand,tag=pot_marker,tag=pot_marker_item_set] at @s if data block ~ ~ ~ {item:{count:2}} if score @s enhanced_pots.item_count matches 99 run function enhanced_pots:summon_pot_overflow
